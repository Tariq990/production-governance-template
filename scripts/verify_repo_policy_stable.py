...
def normalize(root: Path, entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for raw in entries:
        ...
        if identity in seen:
            raise ValueError(f"duplicate baseline identity: {path_text} {digest}")
        seen.add(identity)
        path = root / path_text
        if not path.exists():
        ...
        if not matches:
            raise ValueError(f"baseline snippet not found: {path_text} {digest}")
        if len(matches) > 1:
            raise ValueError(f"ambiguous baseline snippet: {path_text} {matches}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if "--json" in args:
            print(json.dumps({"exit_code": 1, "checks_run": [], "violations_found": True, "normalization_error": str(exc)}, indent=2))
        return 1
