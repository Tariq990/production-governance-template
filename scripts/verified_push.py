...
def git(*args: str) -> str:
    ...
    if completed.returncode != 0:
        raise VerifiedPushFailure(completed.stderr.strip() or completed.stdout.strip())
...
def non_report_dirty() -> list[str]:
    ...
    return [line for line in lines if not line[3:].strip().replace("\\", "/").startswith("reports/")]


def create_proof(head: str, branch: str, report: Path, contract: Path) -> Path:
    directory = proof_dir()
...
def main() -> int:
    ...
    if dirty:
        ...
        return 1
    for path in (REPORT, ROOT / "reports" / "pr-gate.md", ROOT / "reports" / "delivery-report.md"):
        if path.exists():
            path.unlink()
    ...
    completed = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "pr_gate.py"), "--level", "gate", "--contract", str(contract)],
        cwd=ROOT,
        text=True,
        env=environment,
        check=False,
    ...
    if completed.returncode != 0 or not REPORT.exists():
        print(f"BLOCKED: gate failed with exit {completed.returncode}", file=sys.stderr)
        return 1
    report = json.loads(REPORT.read_text(encoding="utf-8"))
    if report.get("run_nonce") != nonce:
        print("BLOCKED: report nonce mismatch", file=sys.stderr)
        return 1
    if report.get("head_sha") != head or report.get("contract_sha256") != contract_hash(contract):
        print("BLOCKED: report identity mismatch", file=sys.stderr)
        return 1
    if report.get("result") != "PASS" or report.get("violations_found") is not False:
        print("BLOCKED: report is not a clean PASS", file=sys.stderr)
        return 1
    dirty = non_report_dirty()
    if dirty:
        print(f"BLOCKED: gate modified tracked files: {dirty[0]}", file=sys.stderr)
        return 1
